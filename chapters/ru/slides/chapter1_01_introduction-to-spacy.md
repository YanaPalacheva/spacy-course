...
то есть позволяет перебирать элементы и получать элемент по индексу. Но об этом чуть позже.
...
Notes: Токены документа - слова или знаки препинания - представлены объектами `Token`.

Чтобы узнать, какой токен стоит на конкретной позиции в тексте,
вы можете получить соответствующий `Token` по индексу из doc.
...
# Получение текста спана с помощью атрибута .text
print(span.text)
```
...
Это лишь вариант представления объекта `Doc`, и он не содержит в себе информации
как таковой.

Чтобы создать подобный объект (спан), можно использовать так называемый срез (slice)
...
Notes: Давайте применим знания на практике и обработаем ваш первый текст с помощью spaCy!
